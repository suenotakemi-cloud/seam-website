...
(function () {
  ...
  function renderRebate() {
    ...
    if (sum) {
      ...
      sum.innerHTML = `<div class="adm-kpis" style="margin-bottom:14px">
          ${tile('補填対象メーカー', D.tiers.length + '社', '達成→1本無料（補填）', '')}
        ...
        <div style="font-size:11.5px;color:var(--ink-3);margin-top:10px"><b>値引きではなく補填</b>＝達成すると<b>翌月の発注で使える1本無料の補填チケット</b>を発行。メーカーは価格を崩さず（現品で奨励）、サロンは実質コスト減・発注集中＋翌月の発注理由になる。補填チケットの1本無料は既存の「メーカー添付（無償現品）」と同じ枠組みで精算。</div>`;
    }
