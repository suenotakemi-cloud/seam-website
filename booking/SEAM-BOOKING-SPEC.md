...
- **salon.town の会員番号**＝`account_id`。**日時17桁＋英数7桁の24文字**（例 `20241027205631236OjejSgX`）。物理カードのQRの中身で、入場受付 `https://seam.site/entrance` がこれを `/entrance/lookup?code=` に投げて氏名を出している（[[seam-entrance-kiosk]]）。
- **入場受付側は一切変更していない**。キオスクは読んだ値をそのまま照会するだけなので、LINEの会員証に同じ値のQRを出せばそのまま通る。**入場受付が使っている jsQR で実際に読めることを3件で確認済**。
- **3種類の番号が1つのスキャン欄で分かれる**（`#co-scan`）：
  | 種類 | 形 | 判定 |
  |---|---|---|
  | 商品 | JAN 13桁 / 8桁 | 上2つに当たらないもの |
  | うちの会員証 | `99`+8桁＝10桁 | `/^99\d{8}$/` |
  | 物理の会員カード | 24文字 | `/^\d{17}[0-9A-Za-z]{7}$/`（`SALON_ID_RE`） |

  15パターンで衝突しないことを検証済（9桁/11桁/23文字/25文字/17桁目が英字などの紛らわしいものを含む）。
- **結び付け**：レジのスキャン欄で物理カードを読むと、未登録なら「このカードを◯◯様に登録しますか？」と出て `POST /member/salon-link` で `members.salon_account_id` に入る。**同じカードが別のお客様に付いていたら409で止める**（付け間違いは入場と会計の両方に響くため）。
- **会員証の見た目**：salon番号があれば **QRを主役**（「入場・お会計 兼用」）＋その下に10桁のCode39（1次元リーダー用）。無ければCode39だけ。
- **⚠️salon APIを叩いていない**：入場受付は `seam_ginza_qr_01` の**1アカウント=1セッション**なので、こちらからログインすると**iPadのセッションが切れて会員名が出なくなる**（[[seam-entrance-kiosk]] の実障害）。氏名や電話からの自動照会はしていない＝**レジで1回読んで結び付ける**方式にしてある。
...
## 6.6 会員ランクとポイント（2026-07-30）

お客様の常連ぶりを**3つの軸**で見て、**いちばん高く出た軸**のランクを差し上げる（甘めに見るのは意図的＝VIP気分を損なわない）。ホットペッパー経由より自社サイトのほうが得になる状態を作るための仕組み。

- **3軸**：①通算のご来店回数 ②直近1年のご来店回数 ③直近1年のご利用金額。既定は スタンダード1% / ブロンズ2%(3回・2回・¥5万) / シルバー3%(6・4・¥10万) / ゴールド5%(12・8・¥20万) / プラチナ7%(24・12・¥40万)。管理画面（設定→👑会員ランクとポイント）で全て変えられる。
- **ポイントの値打ち（二重レート）**：オンライン・店販の商品は **1pt=¥1**、サロン・スパの施術は **1pt=¥0.5**。施術のほうが原価がほぼ人の時間なので値引きの重みを変えている。`ptToYen()` が換算。**施術から先に当てる**（お客様が多く使える）。
- **⚠️ポイント消費の原則**：入力しすぎても**値引きに変わった分だけ**を引く。1pt=¥0.5で奇数ptの端数は返す。1ptだけ入れて¥0になる場合は消費しない。入力欄の上限も「使い切れるpt」に絞る（`maxPt`）。
- **LINE会員のみ**：会員証（`memberCode`）がないお客様にはポイントを付けない（既定ON・管理画面で外せる）。LINE登録の動機にしている。
- **表示**：お客様のLINE会員証にランクバッジ・3軸ゲージ・**「あと◯回で次のランク」**（いちばん近い軸だけを一言で）。レジと予約詳細には**ランクの帯**（`rankStrip`）＝スタッフが施術前にVIPを認識して**名前でお声がけ**できる。
- **API**：`GET /customer/rank?key=`／`GET・POST /rank/config`。設定は D1 `settings` の `rank_config`。

## 6.7 スタッフのスマホと歩合の段（2026-07-30）

`booking/staff.html`（スタッフ用・単一HTML）。自分の売上と自分の予約だけを見る。

- **歩合の段**：「この売上を超えたら、超えた分にこの率」。既定は ¥0/0% → ¥50万/25% → ¥70万/30% → ¥90万/35% → ¥120万/40%。**いちばん上（率0%）がスタートライン**。
- **ゲージ**：ラインに届く前は「歩合まであと◯円」、届いた後は「次の段まであと◯円 → ◯%」。伸ばすほど手取りの増え方が早くなることを見せる。
- **認証**：6桁の合言葉を管理画面（設定→📈歩合の段とスタッフのスマホ）から発行。**発行しなおすと前の合言葉とログイン中の端末は切れる**（端末紛失時の止め方）。トークンは `staff_auth.token`。
- **⚠️`staff.html` の `STAFF` は `index.html` の `STAFF` と id・名前を合わせる**（idがずれると別の人の数字が出る）。
- **API**：`POST /staff/pass`（管理・発行）／`GET /staff/passes`（管理・一覧）／`POST /staff/login`（スタッフ）／`GET /staff/me?token=`（スタッフ・自分の数字と予約）／`GET・POST /comm/config`。設定は D1 `settings` の `comm_config`。

## 6.8 予約の短い空き（スタッフ側だけ・2026-07-30）

予約と予約のあいだの **20〜60分**の空きを見つけ、前後どちらに寄せればまとまった空きになるかを具体的に出す（`gapsOf` / `gapsSoon`）。お願いの文面もその場で作る（`gapAskText`・断りやすい書き方）。今日と明日だけ見せる。

**⚠️特許回避（`booking-patent-avoidance.md`）**：お客様の画面は**一切変えない**。①隙間枠を隠さない ②最大空き時間をお客様に見せない ③短いメニューだけを出す出し分けもしない。**やるのは「スタッフが気づいて人がお願いする」ところまで**（管理側隙間提案＝セーフな形）。

---

## 6.94 salon.town予約が「未確定」扱いだった件（2026-07-31・SaaS非表示とRPA不達の有力原因）

**症状**: 自社経由の予約が salon.town の画面（SaaS）に出ず、RPAもサロンボードに書かない。データ自体はSPA店に正しく入っている。

**原因（有力）**: `/save/reservation` に **`status: 'pending'` を送っていた**。エンジニアのHPBメール取込は status **null** で作っており、そちらは画面にも出てRPAも処理する。CUEPONでは `pending` ＝未確定の扱いと思われる（売上集計も `accept_date` 基準＝T12）。

**直し（2026-07-31デプロイ済み）**:
1. **作成時に status を送らない**（null＝エンジニアの取込と同じ形）
2. **作成直後に必ず承認（accept）を打つ**（`{ accept: true, data: { ids: [id] } }`・T5のparams直下）。失敗しても予約は落とさずログに残す
3. 復旧用に `POST /admin/salon-accept {ids, clearStatus}` を常設（管理トークン）

**⚠️直せなかったもの**: 既に `pending` で作った過去分は、更新APIで status を null にも空文字にも変えられない（`result:true` が返るが無視される）。対象はテスト予約3件のみ（8/5の11:00・14:00=キャンセル済み・17:00）。17:00は削除→作り直しでしか直らない。

**検証待ち**: 新しくスパ予約を1件作り、SaaSの画面に出るかで確定する。出れば同じ原因でRPAも動く可能性が高い。

---

## 6.95 お金の経路の防御（2026-07-31・徹底掃引）

モデル切替後の集中バグ潰しで見つけて直したもの。**全て本番で実測検証済み。**

| # | バグ | 直し方 |
|---|---|---|
| A | `salon_cancel` の記録が **TDZエラーをcatchが握りつぶして一度も書かれていなかった**（見張りが空振り） | 書き込みを値の確定後へ移動。競合テストで記録されることを実測 |
| B | **二重返金の競合**。同じキャンセルリンクを同時に2回叩くと両方が返金し得た | `status='cancelling'` の取り合い（条件付きUPDATEの `meta.changes`）で勝った1本だけ進む。返金失敗時は元のstatusへ戻す＝やり直せる。Squareの冪等鍵も `cancel-<予約ID>` に固定。**同時2本の実測で本処理1・already1** |
| C | **前金だけ取られて予約なし**。決済成功→予約作成が衝突/サーバ拒否で失敗すると、返金されず「承りました」画面のまま予約が消える経路もあった | `POST /pay/refund-orphan`（認証なしだが: どの予約にも紐づかない決済のみ・note「前金」のみ・1時間以内のみ・冪等鍵 `orphan-<決済ID>`）。クライアントは衝突経路と非同期拒否経路の両方から呼び、完了画面が出ていたら選び直しへ戻す。**紐づき済み決済の拒否(409)を実測** |
| D | キャンセル料が**割引前の料金**で計算されていた（前金は割引後で取っている） | `price - r.discount` に統一（cancel-info / cancel 両方） |
| E | **売上・ポイントの保存失敗が黙って消える**。`API.saveCheckout` 等は失敗してもローカルに残るが、次回の `syncRegisterFromD1` がD1で丸ごと置き換えるため記録が消失 | **送信箱**（`stb_outbox`・localStorage・上限200件）。checkout/point/gift/pass/settlement/intake の6種を `saveOrQueue` 経由に。失敗時は赤トースト＋起動時と60秒ごとに自動再送。**D1置き換えの前に必ず送信箱を流す** |

**⚠️残っている構造リスク（意図して受け入れ）**: `MENUS` が画面とサーバに二重管理（28件・価格含め一致を確認済）。価格改定時は**両方を直す**こと。ずれるとキャンセル料と前金の計算が食い違う。
