...
| 商品・在庫 | 商品マスタ（名/価格/バーコード/在庫）・在庫わずか警告・**入荷(仕入)履歴**・**📦棚卸し**・**📥CSV一括取り込み** |
...
- **★Square商品台帳の取り込み（2026-07-30・ショップを自作POSへ移すため）**：`POST /admin/square-catalog`。
  - **実売しているSquare側を商品台帳の正とする**（卸リストの上代ではなく、実際の売値・実際の商品名）。突き合わせは Squareの商品ID → JAN(GTIN) → SKU(メーカー品番) → 名前 の順。`products.sq_id` に保持
