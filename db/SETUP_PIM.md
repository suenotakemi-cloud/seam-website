...
### G2. 商品写真ガイド（種類別の 5 枚）
- `pim/photo-guide.html`: ディーラー EC と同じ見え方に揃えるための、種類（カラー剤の箱／2剤／パーマ剤／ボトル／ジャー／チューブ・袋／スプレー／機器／ヘアマニキュア等／キット／コスメ／小物）ごとの 5 枚の角度と NG 例。印刷可。
- 種類は `pim/photo-guide.js` が **商品のカテゴリ（大 > 中 > 小）と商品名** から自動で当てる（菊池 スマイルの分類は全部対応表にある）。スマホのカメラ画面はその種類の「n枚目: …」を出し、「📖 ガイド」で該当ページが開く。手で種類を変えることもできる。
