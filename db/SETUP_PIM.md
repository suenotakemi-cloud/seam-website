...
- 各写真に **削除** ボタン。画像まとめ登録でまぎれ込んだ不要な写真を、検品しながらその場で消せます（`DELETE /api/pim/images`。後ろの写真が前に詰まる・確認あり）。
- 商品一覧の「写真」で開いた編集画面には **「← 商品一覧に戻る」** が出ます。戻ると、開く前の検索語・絞り込み・ページ・スクロール位置がそのまま復元されます（`index.html?jan=…&back=list` → `import.html#products`。状態は sessionStorage）。
...
### V. 公開前チェック・EC 反映の確認・担当割り・シリーズ並べ（2026-09 追加）
- **公開できる商品**（EC に出してよい条件）= 写真 1 枚以上・商品コードあり・価格あり・撮り直しの写真が無い。出力タブの「公開前チェック → 公開できる商品だけ」、API は `export?…&ready=1`。商品一覧の「公開できない商品だけ」で残りを確認（「公開」列に 可／不可）。進捗タブに「公開できる／できない／価格なし／撮り直しあり／EC 未反映」の数。
- **EC の受け取り確認（ack）**: EC が取り込み終えた JAN を `POST /api/pim/ack { jans:[…] }`（連携キー `api_key` でも可）で知らせると、商品一覧の「EC」列が 済 になる。商品や写真が更新されると自動で 未 に戻る（`ec_synced_at < updated_at`）。EC が「未反映の一覧」を取りに来るなら `GET /api/pim/ack?limit=500`。
- **EC 側の差分取り込みの手順（推奨）**: ① 毎晩 `GET /api/pim/ack` で未反映の JAN を取る → ② `GET /api/pim/export?format=source&since=<前回>` か `products?jan=` で内容を取る（写真は URL のまま） → ③ 取り込めた JAN を `POST /api/pim/ack` → ④ Webhook を受けたときも同じ ①〜③。全件の取り直しは不要。
- **担当割り**: PC 設定 → 担当者一覧 → 「担当」にブランド名かメーカー名（カンマ区切り）。スマホの撮影キューはその人の担当だけになる（「自分の担当だけ」を外せば全部）。API: `staff {action:'assign', id, assign}`、`queue?brands=A,B`。
- **シリーズ並べ**（PC「並び確認」タブ）: ブランドと枚目を選ぶと、その写真を並べて表示。目安の線を重ねて位置・大きさのずれを見つけ、「撮り直しへ」で検品の撮り直し（理由: 位置ずれ）にする → スマホの撮り直しキューに出る。
- **表記ゆれの自動提案**（PC 設定 → 表記の辞書 → 「揺れを探す」）: メーカー・ブランド・カテゴリの値を、空白・全角半角・記号を無視して同じものにまとめ、複数の書き方があるものを「数の多い書き方に揃える」提案として出す。チェックして登録すると既存の商品にも当たる。API: `dict {action:'suggest'}`。
- **画像まとめ登録の名前当て**: ファイル名に JAN が無い画像は、ファイル名（商品名・型番）で商品を検索して候補を出す（「名前から推定」）。候補の選び直し・登録しないも選べる。
- **見本との位置合わせ**（スマホのカメラ）: 見本が重なっているとき、映像の「商品の範囲」を見本と比べて「← 左へ」「もう少し寄る」「✓ 見本とぴったり」を右上に出す（端末内で計算。白い背景が前提）。
