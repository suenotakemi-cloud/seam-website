...
//     'commit' { import_id, products:[...], issues:[...] } → { inserted, updated }  ※300件ずつ呼ぶ
...
import { json, cleanJan, janShapeOk, sanitizeProduct, upsertStmt, nowIso } from './_lib.js';
...
export async function onRequestPost({ request, env }) {
  ...
  if (action === 'check') {
    const jans = Array.from(new Set((Array.isArray(b.jans) ? b.jans : []).map(cleanJan).filter(janShapeOk))).slice(0, 5000);
    const existing = {};
  ...
  if (action === 'begin') {
    ...
    ).bind(nowIso(), String(b.filename || '').slice(0, 200), String(b.source || '').slice(0, 100), JSON.stringify(b.mapping || {}).slice(0, 4000), Math.max(0, parseInt(b.total, 10) || 0)).run();
  ...
  if (action === 'commit') {
    ...
    const ts = nowIso();
    const products = (Array.isArray(b.products) ? b.products : []).slice(0, 500).map(sanitizeProduct).filter((p) => p.jan && janShapeOk(p.jan) && p.name);
    // 挿入/更新の区別（件数報告用）
    const jans = products.map((p) => p.jan);
    const existing = new Set();
    for (let i = 0; i < jans.length; i += 90) {
      const chunk = jans.slice(i, i + 90);
      const rs = await env.DB.prepare('SELECT jan FROM pim_products WHERE jan IN (' + chunk.map(() => '?').join(',') + ')').bind(...chunk).all();
      for (const r of (rs.results || [])) existing.add(r.jan);
    }
    let inserted = 0, updated = 0;
    for (const p of products) { if (existing.has(p.jan)) updated++; else inserted++; }
    const stmts = products.map((p) => upsertStmt(env, p, importId, ts));
    // 注意（未解決の重複）
    ...
    return json({ ok: true, inserted, updated, issues: issues.length });
  }
