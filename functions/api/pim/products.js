...
export async function onRequestPut({ request, env, data }) {
  ...
  if (expected) {
    ...
    ).bind(p.name, p.price, p.tax_included, p.tax_rate, p.price_ex, p.price_in, p.retail_price, p.cost_price, p.amount, p.unit, p.maker, p.brand, p.category, p.description, p.sku, p.jan_valid, ts, by || null, acct, p.jan, expected).run();
